...
CasStrategy.prototype.authenticate = function(req, options) {
  ...
  } else if (req.session.passport !== undefined && req.session.passport.user !== undefined) {
    console.log(req.session);
    self._verify(req.session.passport.user.id, req.session.passport.user, verified);
  }
  else {
